...
function readUrl(): { tab: Tab; env: number; sliders: SliderValues; trendDiscount: number; frame: DemoFrame } {
  ...
  // Default D+6.7 = current generic-ballot average per VoteHub polling avg
  const env = Number(p.get("env") ?? "6.7");
  ...
  return { tab, env: Number.isNaN(env) ? 6.7 : env, sliders: out, trendDiscount, frame };
}

function writeUrl(tab: Tab, env: number, sliders: SliderValues, trendDiscount: number, frame: DemoFrame) {
  const p = new URLSearchParams();
  if (env !== 6.7) p.set("env", String(env));
  for (const s of SLIDER_DEFS) {
...
export default function App() {
  ...
  const onReset = () => { setEnvironment(6.7); setSliders(DEFAULT_SLIDERS); setTrendDiscount(0.5); setFrame("race"); };
