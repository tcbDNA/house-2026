...
function readUrl(): { tab: Tab; env: number; sliders: SliderValues; trendDiscount: number; frame: DemoFrame } {
  ...
  // Default D+7.6 = current generic-ballot average per VoteHub polling avg
  const env = Number(p.get("env") ?? "7.6");
  ...
  return { tab, env: Number.isNaN(env) ? 7.6 : env, sliders: out, trendDiscount, frame };
}

function writeUrl(tab: Tab, env: number, sliders: SliderValues, trendDiscount: number, frame: DemoFrame) {
  const p = new URLSearchParams();
  if (env !== 7.6) p.set("env", String(env));
  for (const s of SLIDER_DEFS) {
...
export default function App() {
  ...
  const onReset = () => { setEnvironment(7.6); setSliders(DEFAULT_SLIDERS); setTrendDiscount(0.5); setFrame("race"); };
